...
func (q *Queries) expand(qs string, args ...interface{}) (string, []interface{}) {
	for i, _ := range args {
		qs = strings.Replace(qs, "$"+strconv.Itoa(i+1), "$$"+strconv.Itoa(i+1), -1)
	}
	...
	for i, v := range args {
		...
		if !ok {
			rn++
			qs = strings.Replace(qs, "$$"+strconv.Itoa(i+1), "$"+strconv.Itoa(rn), -1)
			rArgs = append(rArgs, v)
			continue
		}

		r := reflect.ValueOf(vi.arg)
		...
		for j := 0; j < r.Len(); j++ {
			rn++
			dollar += "$" + strconv.Itoa(rn) + ", "
			rArgs = append(rArgs, r.Index(j).Interface())
		}
		qs = strings.Replace(qs, "$$"+strconv.Itoa(i+1), strings.TrimSuffix(dollar, ", "), -1)
	}
	return qs, rArgs
}

type expandArg struct{ arg interface{} }
{{end}}
...
func (q *Queries) {{.MethodName}}(ctx context.Context, {{.Arg.Pair}}) ([]{{.Ret.Type}}, error) {
  	{{- if $.EmitPreparedQueries}}
	query, args := q.expand({{.ConstantName}}, {{.Arg.Params}})
	rows, err := q.query(ctx, q.{{.FieldName}}, query, args...)
  	{{- else}}
	query, args := q.expand({{.ConstantName}}, {{.Arg.Params}})
